...
class RAGManager:
    ...
    # =========================================================================
    # 🔥 TÉCNICAS DE VANGUARDIA 2024-2025
    # =========================================================================

    def init_vanguard_features(self, rag_name: str = "wikipedia", lazy_bm25: bool = True):
        """
        Inicializar características de vanguardia para un RAG.

        Esto activa:
        - Hybrid Search (Dense + Sparse BM25) - LAZY por defecto
        - Re-ranking con Cross-Encoder
        - Self-RAG (opcional, requiere LLM)
        - CRAG (opcional, requiere web searcher)

        OPTIMIZACIÓN DE MEMORIA:
        - BM25 se carga bajo demanda (lazy_bm25=True)
        - Evita duplicar instancias si ya existe

        Args:
            rag_name: Nombre del RAG a potenciar (default: wikipedia)
            lazy_bm25: Cargar BM25 bajo demanda (default True, ahorra ~10-20GB RAM)
        """
        # Evitar re-inicialización
        if hasattr(self, '_hybrid_engine') and self._hybrid_engine is not None:
            logger.info(f"⚠️ Vanguard features ya inicializadas, saltando")
            return

        if rag_name not in self.rags:
            logger.warning(f"⚠️ RAG '{rag_name}' no encontrado")
            return

        rag = self.rags[rag_name]

        if not rag.index or not rag.documents:
            logger.warning(f"⚠️ RAG '{rag_name}' no tiene índice cargado")
            return

        num_docs = len(rag.documents)
        logger.info(f"🔧 Inicializando vanguard features para '{rag_name}' ({num_docs} docs)")

        # === Hybrid Search (LAZY BM25) ===
        try:
            from core.hybrid_search import HybridSearchEngine
            bm25_path = os.path.join(rag.index_path, "bm25_index.pkl")

            # IMPORTANTE: lazy_bm25=True evita cargar BM25 inmediatamente
            self._hybrid_engine = HybridSearchEngine(
                ...
                lazy_bm25=lazy_bm25  # ⚡ No cargar BM25 hasta que se necesite
            )
            logger.info(f"🔥 Hybrid Search configurado (BM25: {'lazy' if lazy_bm25 else 'loaded'})")
        except ImportError as e:
            logger.warning(f"⚠️ Hybrid Search no disponible: {e}")
            self._hybrid_engine = None

        # === Reranker (ligero, se puede cargar) ===
        try:
            from core.reranker import CachedReranker
            self._reranker = CachedReranker(preset="balanced", cache_size=500)
            logger.info("🎯 Re-ranker activado")
        except ImportError as e:
            logger.warning(f"⚠️ Re-ranker no disponible: {e}")
            self._reranker = None

        # Self-RAG y CRAG se inicializan bajo demanda (requieren LLM)
        self._self_rag = None
        self._crag = None
    ...
    def search_hybrid(
        ...
        """
        Búsqueda híbrida Dense (FAISS) + Sparse (BM25).

        Args:
            query: Consulta
            k: Número de resultados
            alpha: Peso de búsqueda densa (0-1). 0.6 = favorece semántica
        ...
        if not hasattr(self, '_hybrid_engine') or self._hybrid_engine is None:
            logger.warning("⚠️ Hybrid Search no inicializado, usando búsqueda normal")
            return self.search(query, k=k)

        results = self._hybrid_engine.search(query, k=k, alpha=alpha, fusion_method=fusion_method)
    ...
    def search_with_rerank(
        ...
        """
        Búsqueda con re-ranking usando Cross-Encoder.

        Flujo:
        1. Recuperar k*multiplier candidatos
        2. Re-rankear con Cross-Encoder
        3. Devolver top-k

        Args:
            query: Consulta
            k: Resultados finales
            candidates_multiplier: Factor de candidatos (default 3x)
            use_hybrid: Si usar Hybrid Search para candidatos

        Returns:
            Lista de SearchResult re-rankeados
        """
        if not hasattr(self, '_reranker') or self._reranker is None:
            logger.warning("⚠️ Reranker no inicializado, usando búsqueda normal")
            return self.search_hybrid(query, k=k) if use_hybrid else self.search(query, k=k)

        # 1. Obtener candidatos
        num_candidates = k * candidates_multiplier
        if use_hybrid and hasattr(self, '_hybrid_engine') and self._hybrid_engine:
            candidates = self.search_hybrid(query, k=num_candidates)
        else:
            candidates = self.search(query, k=num_candidates)

        if not candidates:
            return []

        # 2. Preparar docs para reranker
        docs_for_rerank = [
            {
                'document': r.document,
                'score': r.score,
        ...
        # 3. Re-rankear
        reranked = self._reranker.rerank(query, docs_for_rerank, top_k=k)
        ...
        for r in reranked:
            search_results.append(SearchResult(
                document=r.document,
                score=r.rerank_score,  # Usar score de rerank
                metadata=r.metadata or {},
                source_rag="reranked"
            ))

        logger.info(f"🎯 Reranking: {len(candidates)} candidatos → {len(search_results)} resultados")
        return search_results
    ...
    def search_vanguard(
        ...
        """
        Búsqueda de vanguardia completa: Hybrid + Rerank.

        Args:
            query: Consulta
            k: Resultados
            mode: "hybrid", "rerank", o "full" (ambos)
        ...
        if mode == "hybrid":
            results = self.search_hybrid(query, k=k)
            metrics["hybrid_used"] = True
        elif mode == "rerank":
            results = self.search_with_rerank(query, k=k, use_hybrid=False)
            metrics["rerank_used"] = True
        else:  # full
            results = self.search_with_rerank(query, k=k, use_hybrid=True)
            metrics["hybrid_used"] = True
            metrics["rerank_used"] = True

        metrics["results_count"] = len(results)
        metrics["top_score"] = results[0].score if results else 0

        return results, metrics

    def init_self_rag(self, llm_interface: Any):
        """
        Inicializar Self-RAG con interfaz LLM.
        ...
        try:
            ...
            logger.warning(f"⚠️ Self-RAG no disponible: {e}")

    def init_crag(self, llm_interface: Any, web_searcher: Any = None):
        """
        Inicializar CRAG (Corrective RAG).

        Args:
            llm_interface: Interfaz al modelo de lenguaje
            web_searcher: Cliente para búsqueda web (opcional)
        """
        try:
            from core.crag import CorrectiveRAG
            ...
            logger.warning(f"⚠️ CRAG no disponible: {e}")

    def generate_self_rag(self, query: str) -> Dict[str, Any]:
        """
        Generar respuesta con Self-RAG (auto-validación).

        Args:
            query: Consulta

        Returns:
            Dict con response, confidence, métricas
        """
        if not hasattr(self, '_self_rag') or self._self_rag is None:
            return {
                "error": "Self-RAG no inicializado. Llamar init_self_rag() primero.",
                "response": None
            }

        result = self._self_rag.generate(query)

        return {
            "response": result.response,
            "confidence": result.confidence,
            "retrieval_decision": result.retrieval_decision.value,
            "support_level": result.support_level.value,
            "usefulness": result.usefulness_level.value,
            "docs_used": len(result.documents_used),
            "iterations": result.iterations,
            "metadata": result.metadata
        }

    def generate_crag(self, query: str, k: int = 5) -> Dict[str, Any]:
        """
        Generar respuesta con CRAG (corrección automática).
        ...
        Returns:
            Dict con response, correction_action, métricas
        """
        if not hasattr(self, '_crag') or self._crag is None:
            return {
                "error": "CRAG no inicializado. Llamar init_crag() primero.",
                "response": None
            }

        result = self._crag.retrieve_and_generate(query, k=k)

        return {
            "response": result.response,
            "confidence": result.confidence,
            "local_quality": result.local_quality,
            "correction_action": result.correction_action.value,
