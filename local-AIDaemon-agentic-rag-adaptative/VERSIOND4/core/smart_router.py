...
# =============================================================================
# 🛡️ DETECCIÓN DE RUTAS EXPLÍCITAS (para bypasear RAG/Wikipedia)
# =============================================================================

import re

# Patrones para detectar rutas explícitas de filesystem
PATH_PATTERNS = [
    # Unix/Mac absolute paths
    r'(?:^|\s)(/(?:Users|home|var|tmp|opt|etc|usr|Library|Applications)[^\s,;:]*)',
    r'(?:^|\s)(~/[^\s,;:]*)',  # Home directory
    r'(?:^|\s)(\./[^\s,;:]+)',  # Relative with ./
    r'(?:^|\s)(\.\.(?:/[^\s,;:]+)+)',  # Relative with ../
    # Extensiones de archivo explícitas con path
    r'(?:^|\s)([^\s]+\.(?:log|txt|py|js|json|md|sh|yaml|yml|cfg|conf|ini)[^\s]*)',
]

# Palabras clave que indican operación de filesystem (no Wikipedia)
FILESYSTEM_KEYWORDS = [
    'logs', 'archivo', 'archivos', 'carpeta', 'carpetas', 'directorio',
    'folder', 'file', 'files', 'directory', 'path', 'ruta',
    'listar', 'lista', 'leer', 'lee', 'abrir', 'abre', 'ver',
    'contenido', 'mostrar', 'muestra', 'revisar', 'revisa',
    'ls', 'cat', 'head', 'tail', 'grep', 'find',
]


def detect_explicit_path(query: str) -> Optional[str]:
    """
    Detecta si hay una ruta explícita de filesystem en la consulta.

    Returns:
        La ruta detectada o None si no hay
    """
...
def should_bypass_rag(query: str) -> Tuple[bool, Optional[str], str]:
    """
    Determina si debemos bypasear el RAG y usar filesystem directamente.

    Returns:
        (should_bypass, detected_path, reason)
    """
    query_lower = query.lower()

    # 1. Detectar path explícito
    detected_path = detect_explicit_path(query)

    if detected_path:
        # Path explícito + keyword de filesystem = BYPASS seguro
        has_fs_keyword = any(kw in query_lower for kw in FILESYSTEM_KEYWORDS)
        if has_fs_keyword:
            return True, detected_path, "Path explícito + keyword filesystem"

        # Path explícito sin keyword = probablemente filesystem
        # (ej: "/Users/.../logs" sin "listar" pero claramente es filesystem)
        if detected_path.startswith('/') or detected_path.startswith('~'):
            return True, detected_path, "Path absoluto detectado"

    # 2. Comando directo de filesystem (sin path pero claramente fs)
    fs_commands = ['ls ', 'ls -', 'cat ', 'head ', 'tail ', 'grep ', 'find ']
    if any(query_lower.startswith(cmd) for cmd in fs_commands):
        return True, None, "Comando shell directo"

    return False, None, "No es operación de filesystem"
...
class SmartRouter:
    ...
    def route(self, query: str, context: str = "") -> RouteResult:
        ...
        # =========================================================
        # 🛡️ PASO 0: DETECCIÓN DE RUTAS EXPLÍCITAS (BYPASS RAG)
        # =========================================================
        # Si hay un path explícito, NO buscar en Wikipedia/RAG
        bypass_rag, detected_path, bypass_reason = should_bypass_rag(query)

        if bypass_rag and self.verbose:
            print(f"\n🛡️ BYPASS RAG ACTIVADO:")
            print(f"   📂 Path: {detected_path or 'N/A'}")
            print(f"   💡 Razón: {bypass_reason}")

        # Clasificar intención
        classification = self.classifier.classify(query)

        # Si detectamos path explícito, forzar intención a ACTION/FILESYSTEM
        if bypass_rag and classification.intent == Intent.INFORMATIVE:
            if self.verbose:
                print(f"   ⚠️ Overriding INFORMATIVE → ACTION/FILESYSTEM (ruta explícita)")
            # Modificar clasificación para forzar filesystem
            classification.intent = Intent.ACTION
            classification.action_type = ActionType.FILESYSTEM
            if detected_path:
                classification.entities["paths"] = [detected_path]
                classification.entities["explicit_path"] = detected_path
        ...
        if self.verbose:
            ...
            if bypass_rag:
                print(f"   🛡️ RAG Bypass: ✅ ({bypass_reason})")
    ...
    def _handle_informative(self, classification: ClassificationResult, context: str) -> str:
        ...
        # 🛡️ Seguridad extra: verificar que no hay paths que se hayan colado
        bypass, path, reason = should_bypass_rag(classification.query)
        if bypass:
            logger.warning(f"⚠️ Path detectado en query informativa, redirigiendo a filesystem: {path}")
            # Redirigir a filesystem handler
            classification.intent = Intent.ACTION
            classification.action_type = ActionType.FILESYSTEM
            if path:
                classification.entities["paths"] = [path]
            return self._handle_action(classification, context)
