...
Responde de forma precisa, útil y concisa."""
        ...
        self._status_lock = threading.Lock()  # FIX: Proteger cambios de estado
        ...
        self._metrics_lock = threading.Lock()  # FIX: Proteger métricas

        # Buffer de respuesta actual (protegido por lock)
        self._current_response_buffer = []
        self._buffer_lock = threading.Lock()  # FIX: Proteger buffer compartido
    ...
    def status(self) -> DaemonStatus:
        """Estado actual del daemon (thread-safe)"""
        with self._status_lock:
            return self._status

    @property
    def is_ready(self) -> bool:
        """Daemon listo para recibir consultas (thread-safe)"""
        with self._status_lock:
            return self._status == DaemonStatus.READY

    def _set_status(self, new_status: DaemonStatus):
        """Cambiar estado de forma thread-safe"""
        with self._status_lock:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self._set_status(DaemonStatus.READY)
    ...
    def _process_request(self, request: DaemonRequest):
        """Procesar una solicitud (thread-safe)"""
        start_time = time.time()
        self._set_status(DaemonStatus.BUSY)
        ...
        try:
            ...
            # Actualizar métricas (thread-safe)
            with self._metrics_lock:
    ...
    def stop(self):
        """Detener el daemon (solo una vez, thread-safe)"""
        # Evitar múltiples stops (thread-safe)
        with self._status_lock:
        ...
        # Estado ya fue cambiado al inicio del método
    ...
    def get_metrics(self) -> Dict[str, Any]:
        """Obtener métricas del daemon (thread-safe)"""
        with self._metrics_lock:
            uptime = 0.0
        ...
        # Status fuera del lock de métricas (usa su propio lock)
        with self._status_lock:
        ...
        return {
            ...
            "success_rate": (
                metrics_copy["successful_requests"] / max(1, metrics_copy["total_requests"])
            ),
            "total_tokens": metrics_copy["total_tokens"],
