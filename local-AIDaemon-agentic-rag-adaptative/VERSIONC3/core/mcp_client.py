...
class MCPClient:
    ...
    def start_server(self, name: str, config: Dict) -> bool:
        ...
        try:
            ...
            # Esperar a que el proceso inicie y esté listo
            time.sleep(1.0)
    ...
    def _initialize_server(self, server_name: str) -> bool:
        ...
        if response and 'result' in response:
            ...
            # Esperar un momento para que el servidor procese la notificación
            time.sleep(0.3)
    ...
    def _discover_tools(self, server_name: str) -> List[MCPTool]:
        ...
        logger.debug(f"[discover_tools] Enviando: {request}")
        response = self._send_request(server_name, request)
        logger.debug(f"[discover_tools] Respuesta: {str(response)[:200] if response else 'None'}")

        tools = []
        if response and 'result' in response:
            tool_list = response['result'].get('tools', [])
            logger.debug(f"[discover_tools] Tools en respuesta: {len(tool_list)}")
            ...
            logger.debug(f"[discover_tools] Sin result en respuesta")
    ...
    def _send_request(self, server_name: str, request: Dict, timeout: float = None) -> Optional[Dict]:
        ...
        El protocolo MCP usa JSON Lines: un JSON por línea terminado en \n
        El servidor puede enviar requests propios (como roots/list) que debemos manejar.
        ...
        try:
            # Serializar y enviar con JSON Lines format
            request_bytes = (json.dumps(request) + '\n').encode('utf-8')
            server.process.stdin.write(request_bytes)
            server.process.stdin.flush()

            # Leer respuestas hasta obtener la que corresponde a nuestro request
            import select
            ...
            for attempt in range(max_attempts):
                ...
                # Si es una request del servidor (tiene 'method' sin 'result'), responder
                if 'method' in response and 'result' not in response:
                    server_request_id = response.get('id')
                    method = response['method']
                    logger.debug(f"[MCP] Servidor solicitó: {method}")

                    # Manejar requests del servidor
                    if method == 'roots/list':
                        # Responder con lista de roots vacía o el directorio permitido
                        roots_response = {
                            "jsonrpc": "2.0",
                            "id": server_request_id,
                            "result": {"roots": []}
                        }
                        self._send_raw(server_name, roots_response)
                    else:
                        # Responder con error para requests no soportados
                        error_response = {
                            "jsonrpc": "2.0",
                            "id": server_request_id,
                            "error": {"code": -32601, "message": f"Method not found: {method}"}
                        }
                        self._send_raw(server_name, error_response)

                    # Continuar leyendo para obtener nuestra respuesta
                    continue
                ...
                logger.debug(f"[MCP] Respuesta con id={response.get('id')}, esperando id={expected_id}")

            logger.warning(f"⚠️ No se encontró respuesta para id={expected_id}")
            return None
    ...
    def _send_raw(self, server_name: str, message: Dict):
        """Enviar mensaje raw sin esperar respuesta"""
        if server_name not in self.servers:
            return
        server = self.servers[server_name]
        if server.process and server.process.poll() is None:
            try:
                ...
                logger.debug(f"Error enviando raw: {e}")

    def _send_notification(self, server_name: str, notification: Dict):
        """Enviar notificación (sin esperar respuesta) con JSON Lines format"""
        if server_name not in self.servers:
            return
        ...
        try:
            # Usar formato JSON Lines (JSON + newline)
