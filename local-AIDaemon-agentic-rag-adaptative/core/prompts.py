...
Comienza con una pregunta reflexiva:"""
    ...
    def select_style(cls, query: str, complexity: int = None) -> PromptStyle:
        ...
        # Palabras clave que sugieren CoT
        cot_keywords = ['explica', 'explain', 'por qué', 'why', 'cómo funciona', 'how does', 'razona', 'reason']
        if any(kw in query_lower for kw in cot_keywords):
            return PromptStyle.COT

        # Tareas que requieren acción
        action_keywords = ['busca', 'search', 'encuentra', 'find', 'calcula', 'calculate', 'haz', 'do']
        if any(kw in query_lower for kw in action_keywords):
            return PromptStyle.REACT

        # Preguntas filosóficas/reflexivas
        socratic_keywords = ['qué opinas', 'what do you think', 'debería', 'should', 'es correcto', 'is it right']
        if any(kw in query_lower for kw in socratic_keywords):
            return PromptStyle.SOCRATIC

        # Si hay complejidad alta, usar CoT
        if complexity and complexity >= 4:
            return PromptStyle.COT
